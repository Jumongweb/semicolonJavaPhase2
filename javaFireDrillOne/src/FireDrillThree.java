...
public class FireDrillThree {
    ...
    public int[] taskTwelve(int[] array) {
        ...
        for (int count = 0; count < array.length; count++) {
            if (count % 2 == 0) {
                evenIndexArray[counter] = array[count];
            } else if(count % 2 == 1){
                oddIndexArray[counter] = array[count];
                counter++;
            }
        }

        // Check if the length of evenIndexArray is greater than 0 before swapping
        if (evenIndexArray.length > 0) {
            for (int index = 0; index < evenIndexArray.length; index++) {
                int temp = evenIndexArray[index];
                evenIndexArray[index] = oddIndexArray[index];
                oddIndexArray[index] = temp;
            }
        }
